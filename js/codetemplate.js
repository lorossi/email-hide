function codeTemplate(options) {
  ...
  if (options.max_iterations == 0) {
    js_code = `window.onload=function ${options.function_name}(){${newl}${tab}document.querySelector("${options.encoded_container}").innerHTML="${options.encoded_email}";${newl}}`;
  }
  else if (options.max_iterations == 1) {
    js_code = `window.onload=function ${options.function_name}(){${newl}${tab}let ${options.container_variable}=atob("${options.encoded_container}");${newl}${tab}let ${options.email_variable}=atob("${options.encoded_email}");${newl}${tab}document.querySelector(${options.container_variable}).innerHTML=${options.email_variable};${newl}}`;
  } else {
